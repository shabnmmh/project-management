...
const SubmitButton = ({ loading, children }) => (
  <button
    ...
    style={{
      background: loading ? "#F48FB5" : "#E91E8C",
...
const LoginWithPassword = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      {error && (
        <div
          className="bg-red-50 text-red-600 p-3 rounded-xl text-xs border border-red-100"
      ...
      <div className="flex items-center justify-center gap-2 mb-2">
        {[1, 2, 3].map((s) => (
          <div key={s} className="flex items-center gap-2">
            <div
              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${step >= s ? "text-white" : "bg-gray-100 text-gray-400"}`}
            ...
            {s < 3 && (
              <div
                className={`w-8 h-0.5 rounded ${step > s ? "bg-primary-400" : "bg-gray-200"}`}
              />
            )}
          </div>
        ))}
...
const LoginPage = () => {
  ...
  return (
    <div
      className="min-h-screen flex items-center justify-center bg-[url('/login-bg3.png')] bg-cover"
      dir="rtl"
    >
      {/* Card */}
